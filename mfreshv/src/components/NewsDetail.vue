<template>
      <div class="news_details">
          <h2>{{title}}</h2>
          <span>发布时间：{{ pubTimeFilter(pubTime)}}</span>
          <div class="news_content">



            <!--<p>9月，苹果在发布会上正式发布了iPhone 6S和iPhone 6S Plus，玫瑰金、土豪金的配色再次刺激了时尚筒子们的 神 经.如此靓丽的 定远远不够，家居装饰也要“壕”一次，尤其是雾霾天气下家庭必备的空气净化器。近日净美仕在行业率先推出“千金”M8088A彩色版，可谓为空气净化器产业树立了个性化品牌的标杆。</p>&ndash;&gt;-->
    ...
    export default {
      ...
      methods:{
        showDetail:function (num) {
          var url = 'http://127.0.0.1:8888/news/detail?nid='+num;
          this.$http.get(url).then(function(res){
            console.log(res.body);
            this.title = res.body.title;
            console.log(this.title);
            this.pubTime = res.body.pubTime;
            this.content  = res.body.content;

            this.$news_content.html(this.content);
      ...
      watch:{
        ...
        subTime:function (oldValue,newValue) {
          console.log(arguments);
        }
      }
    }
